    ...
    def get_available_events(self, region):
        try:
            response = self.client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a helpful ticket booking assistant. Return event data in the exact JSON format specified."},
                    {"role": "user", "content": f"List available events in {region} as JSON with name, location, date, time, tickets_available, price, currency, status, organizer, category, and description. Return only the JSON."}
                ]
            )
    ...
    def get_event_details(self, event_name: str) -> Optional[Dict]:
        ...
        try:
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are a ticket booking assistant. Provide detailed event information in JSON format."},
    ...
    def validate_booking(self, event_name, date, num_tickets):
        try:
            response = self.client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a helpful ticket booking assistant."},
                    {"role": "user", "content": f"Validate if {num_tickets} tickets are available for {event_name} on {date}. Return only 'true' or 'false'."}
                ]
            )
    ...
    def get_booking_policies(self, event_name):
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a helpful ticket booking assistant."},
                    {"role": "user", "content": f"Get booking policies for {event_name} as JSON with cancellation_policy, payment_policy, and refund_policy. Return only the JSON."}
                ]
            )
